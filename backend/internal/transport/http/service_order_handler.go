...
// Non-administrators only see orders assigned to them.
...
func (h ServiceOrderHandler) Find(writer http.ResponseWriter, request *http.Request) {
	...
	order, err := h.order.Find(request.Context(), request.PathValue("serviceOrderId"), identity.Role, identity.UserID)
	if err != nil {
		failure(writer, err)
		return
	}
