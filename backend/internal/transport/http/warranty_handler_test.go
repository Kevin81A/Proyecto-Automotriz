...
func TestListWarrantyIsForbiddenForATechnician(t *testing.T) {
	handler := newWarrantyHandler(t, newFakeWarrantyStore())
	request := asCaller(
		httptest.NewRequest(http.MethodGet, "/api/warranty", nil), domain.RoleTechnician,
	)
	recorder := httptest.NewRecorder()
	handler.List(recorder, request)
	if recorder.Code != http.StatusForbidden {
		t.Fatalf("a technician must not list warranties, got %d", recorder.Code)
	}
}
