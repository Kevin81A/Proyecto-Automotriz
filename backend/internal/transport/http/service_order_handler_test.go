...
func TestAdvanceIsForbiddenForUnassignedTechnician(t *testing.T) {
	orders := newFakeOrderStore(orderFixture(t, "order-1", domain.StatusReceived))
	assignments := &fakeAssignmentStore{}
	techStore := newFakeTechnicianStore(t, "technician-1", "user-1")
	...
	// Caller is technician user-2, but order has no assignment (or assigned to another)
	...
	if recorder.Code != http.StatusForbidden {
		t.Fatalf("an unassigned technician must receive 403 Forbidden on advance, got %d body %s", recorder.Code, recorder.Body.String())
	}
}
